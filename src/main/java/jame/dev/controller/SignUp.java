...
public class SignUp {
    ...
    * passing the values from the given {@link InfoUserDto} object and other default values.
    * The token for verification si generated by the class {@link TokenGenerator}.
    * Then, a {@link Runnable} anonymous function is defined to sends the email verification
    * performed by a {@link Thread} more in concrete, a virtual Thread {@code Thread.ofVirtual().start(Runnable r)}
    * and evaluates if this email is correctly sent, then {@code checkVerification(String token)}
    * is called to perform the verification and the insertion in a successful case.
    *
    ...
    * and updates the state of verification from the user to true and then saves it
    * into the db.
    *
    * @param token a random cryptographic secure token.
    */
