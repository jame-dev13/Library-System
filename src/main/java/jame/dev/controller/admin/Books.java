...
public class Books {
   ...
   private final CRUDRepo<BookEntity> repo = new BookService();
   ...
   private void tableConfig() {
      //columns
      colUuid.setCellValueFactory(data ->
      ...
      colEditorial.setCellValueFactory(data ->
      ...
      colPubDate.setCellValueFactory(data ->
              new SimpleObjectProperty<>(data.getValue().getPubDate()));
      colPages.setCellValueFactory(data ->
              new SimpleIntegerProperty(data.getValue().getNumPages()).asObject());
      colGenre.setCellValueFactory(data ->
              new SimpleObjectProperty<>(data.getValue().getGenre()));
      //table
         ...
         if (l.getClickCount() == 2) {
            repo.findByUuid(this.selectedUuid)
                    .ifPresentOrElse(this::loadPopOver,
                            () -> alert.errorAlert("No value founded."));
         }
      });
   }
   ...
   private void handleSave(ActionEvent e) {
      try {
         ...
         if (bookNameSet.add(book.getTitle())) {
            this.save(book);
            alert.infoAlert("Book saved.");
            return;
         }
         alert.warningAlert("Can't duplicate books");

      } catch (NullPointerException ne) {
         alert.errorAlert("Can't have empty fields.");
      ...
      Optional.ofNullable(selectedUuid)
              .ifPresent(uuid -> this.repo.findByUuid(uuid)
                      .ifPresentOrElse(book -> {
                                 this.update(book);
                                 //confirmation
                                 alert.updateConfirmAlert(String.format("Book: [%s] Updated!", book.getTitle()));
                              },
                              () -> alert.errorAlert("Not Found!")
                      )
              );
      this.btnClear.fire();
   }
   ...
   private void handleDelete(ActionEvent e) {
      Optional.ofNullable(selectedUuid).ifPresent(uuid ->
              alert.deleteConfirmAlert("¿Do you want to delete this book record?")
                      .ifPresent(response -> {
   ...
   private void loadPopOver(BookEntity book) {
      try {
         ...
         alert.errorAlert("Can't load the popOver.");
